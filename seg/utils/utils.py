...
def load_image(name):
    image = rasterio.open(name) 

    mask = image.dataset_mask()

    #read in as np arrays
    red = image.read(1)
    green = image.read(2)
    blue = image.read(3)
...
def load_label(name,class_num=2):
    image = rasterio.open(name) 
    ...
    if class_num==2:
        np.place(ann,ann>0,1)
    else:
        np.place(ann,ann==2,1)
        np.place(ann,ann==3,2)
