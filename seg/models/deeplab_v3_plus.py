...
class DeepLabV3Plus(Network):
    ...
    def _deeplab_v3_plus(self, inputs):
        ...
        x = custom_layers.Concatenate(out_size=self.aspp_size)([x, c1])
        ...
        output_seg = layers.UpSampling2D(size=(4, 4), interpolation='bilinear',name='seg')(x) 

        x = tf.reshape(c5,[-1,16*16*2048])
        
        
        y = tf.reshape(output_seg,[-1,256*256*2])
        output_feature = tf.concat([x,y],-1,name='feature')
       
        return models.Model(inputs, [output_seg, output_feature], name=self.version)
